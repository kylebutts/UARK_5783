...
\begin{frame}{Random Allocation ensures comparability}
  ...
  \begin{quote}
    ...
    \textnormal{—Ronald Fisher, The Design of Experiments (1935)}
  \end{quote}
  ...
  \begin{quote}
    ...
    \textnormal{—Austin Bradford Hill, The Clinical Trial (1952)}
  \end{quote}
...
%     TODO: Quick assignment with this? 
...
\begin{frame}{Statistical vs. Causal Inference}
  There are two different questions we could ask about the relationship between $X$ and $Y$:

  \pause
  \bigskip
  \alert{Causal inference} is comparing two counterfactual worlds:
  \begin{itemize}
    \item Take a person and compare the world where they have a larger $X$ to the world where they have a smaller $X$
  \end{itemize}

  \pause
  \bigskip
  \alert{Statistical inference} (prediction) is comparing two people in this world:
  \begin{itemize}
    \item Compare two people, one with a higher $X$
  \end{itemize}
\end{frame}

\begin{frame}{Statistical vs. Causal Inference}{The hospital paradox}
  Say you are trying to predict whether someone dies:
  \begin{itemize}
    \item Compare two people: one in the hospital with heart pains and one who is not. Who is more likely to die?
  \end{itemize}

  \pause
  \bigskip
  The person \emph{in} the hospital, of course!
  \begin{itemize}
    \item Hospitals are \emph{correlated} with death because sick people go there
  \end{itemize}

  \pause
  \bigskip
  But take \emph{one person} with heart pains: are they more likely to die if they were in the hospital or out of the hospital?
  \begin{itemize}
    \item Out of the hospital! Being in the hospital \emph{causes} them to be \emph{less} likely to die
  \end{itemize}
\end{frame}
    ...
    \begin{column}{.6\textwidth}\vspace*{-\bigskipamount}
      \begin{center}
        ...
        @{}} 
          ...
          1 & 1 & \$1200 & \$1200 & \only<1>{\$1225}\only<2>{\color{purple} \textbf{?}} \\
          2 & 0 & \$1000 & \only<1>{\$950}\only<2>{\color{purple} \textbf{?}} & \$1000 \\
          3 & 1 & \$1150 & \$1150 & \only<1>{\$1150}\only<2>{\color{purple} \textbf{?}}\\
          & & \vdots & & \\
          n & 0 & \$1100 & \only<1>{\$1090}\only<2>{\color{purple} \textbf{?}} & \$1100 \\
...
\begin{frame}{Notions of Counterfactual}
  The job of causal inference is to \emph{make \alert{causal assumptions}} about the counterfactual world to take our best guess at the \emph{missing counterfactual outcome}
  \begin{itemize}
...
\begin{frame}{Using Observed Data to Impute the Counterfactual}
\adjustbox{max width=0.9\textwidth, max height=0.8\textheight, center}{
  \begin{tikzpicture}[
    box/.style={draw, rectangle, minimum width=4cm, minimum height=1.6cm, align=center, inner sep=6pt},
    observed/.style={box, thick},
    unobserved/.style={box, dashed, thick, text=gray!90},
    data/.style={box, very thick}
  ]

  % Potential outcomes boxes
  \node[observed] (treated) at (-4,1.4) {
    \textbf{\large Observed Treated Outcome} \\[0.3cm]
    $Y_i(1)$ \\[0.2cm]
    {\small\itshape Bob's outcome had they} \\
    {\small\itshape been treated in this period}
  ...
  \node[unobserved] (untreated) at (4,1.4) {
    \textbf{\large Unobserved Untreated Outcome} \\[0.3cm]
    $Y_i(0)$ \\[0.2cm]
    {\small\itshape Bob's outcome had they} \\
    {\small\itshape not been treated in this period}
  ...
  % Observed data box below
  \node[data] (data) at (-2,-3) {
    \textbf{\large Other Observed Untreated Outcomes} \\[0.3cm]
    $\left\{ Y_j(0) : D_j = 0 \right\}$ \\[0.2cm]
    {\small\itshape Use other people's outcome to ``guess'' Bob's } \\
    {\small\itshape missing counterfactual.}
  };
  ...
  % Squiggly line from observed data to the missing counterfactual
  \draw[->, very thick, purple, bend right=20, decorate, decoration={snake, amplitude=2.5pt, segment length=12pt}, ]
    (data.east) to node[midway, right] {\includegraphics[height=0.8cm]{figures/1f91e.png}} (untreated.south);
...
\begin{frame}{Conditional ATE versus ATE}
  ...
  $$
    \tau_{\text{ATE}} = \expec{\tau_i} = \mathbb{E}\bigg[\  \expec{\tau_i}{X_i = x}\  \bigg]
  $$
  \begin{itemize}
  ...
  $$
    \tau_{\text{ATT}} = \expec{\tau_i}{D_i = 1} = \expec{\tau_{\text{CATE}}(x)}{D_i = 1} = \expec{\expec{\tau_i}{X_i = x}}{D_i = 1}
  $$
\end{frame}
...
\begin{frame}{`Balance Tables'}
  Typically, regress pre-determined characteristics on the treatment dummy
  \begin{itemize}
    \item This allows us to ask if the average $X_i$ is the same for the treated and control groups
  \end{itemize}
\end{frame}


\begin{frame}{Balance Table example (NSW Experiment)}
  \begin{adjustbox}{max width = 0.8\textwidth, center}
    ...
    \midrule
    Age & 24.63 & 24.45 & 0.18 \\
    % & (6.69) & (6.59) & (11.05) \\
    Years of School & 10.38 & 10.19 & 0.19 \\
    % & (1.82) & (1.62) & (2.87) \\
    Proportion High School Dropouts & 0.73 & 0.81 & -0.08 \\
    % & (0.44) & (0.39) & (0.46) \\
    Proportion Married & 0.17 & 0.16 & 0.01 \\
    % & (0.37) & (0.36) & (0.45) \\
    Proportion Black & 0.80 & 0.80 & 0.00 \\
    % & (0.40) & (0.40) & (0.26) \\
    Proportion Hispanic & 0.09 & 0.11 & -0.02 \\
    % & (0.29) & (0.32) & (0.26) \\
    Real Earnings in 1975 (thousand) & 3.07 & 3.03 & 0.04 \\
    % & (4.87) & (5.20) & (9.27) \\
    Proportion Unemployed in 1975 & 0.37 & 0.42 & -0.05 \\
    % & (0.48) & (0.49) & (0.31) \\
    \midrule

    \textbf{\#\ Observations} & \textbf{297} & \textbf{425} & \textbf{-128} \\
    \bottomrule
    \end{tabular}
  \end{adjustbox}
\end{frame}
...
\begin{frame}{LaLonde Example (Imbens and Xu)}
  \begin{adjustbox}{max width = 0.8\textwidth, center}
    \begin{tabular}{lccc}
    \toprule
    & \multicolumn{2}{c}{\textbf{Experimental}} & \textbf{LaLonde Comparison Group} \\
    \cmidrule(lr){2-3} \cmidrule(lr){4-4}
    & Treated & Control & CPS-SSA-1 \\
    
    \midrule
    Age & 24.63 & 24.45 & 33.23 \\
    % & (6.69) & (6.59) & (11.05) \\
    Years of School & 10.38 & 10.19 & 12.03 \\
    % & (1.82) & (1.62) & (2.87) \\
    Proportion High School Dropouts & 0.73 & 0.81 & 0.30 \\
    % & (0.44) & (0.39) & (0.46) \\
    Proportion Married & 0.17 & 0.16 & 0.71 \\
    % & (0.37) & (0.36) & (0.45) \\
    Proportion Black & 0.80 & 0.80 & 0.07 \\
    % & (0.40) & (0.40) & (0.26) \\
    Proportion Hispanic & 0.09 & 0.11 & 0.07 \\
    % & (0.29) & (0.32) & (0.26) \\
    Real Earnings in 1975 (thousand) & 3.07 & 3.03 & 13.65 \\
    % & (4.87) & (5.20) & (9.27) \\
    Proportion Unemployed in 1975 & 0.37 & 0.42 & 0.11 \\
    % & (0.48) & (0.49) & (0.31) \\
    \midrule

    \textbf{\#\ Observations} & \textbf{297} & \textbf{425} & \textbf{15,922} \\
    \bottomrule
    \end{tabular}
  \end{adjustbox}
\end{frame}
