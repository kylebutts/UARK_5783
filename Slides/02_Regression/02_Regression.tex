...
%   The prediction error of the conditional expectation function is given by $\varepsilon_i = y_i - m(\bm{X}_i)$. For any $x$, we have 
%   \begin{align*}
...
%   \begin{align*}
%     &\expec{\left( y_i - f(\bm{X}_i) \right)^2} = \expec{\left( y_i - m(\bm{X}_i) + m(\bm{X}_i) - f(\bm{X}_i) \right)^2} \\ \pause
%     &\quad= \expec{\left( y_i - m(\bm{X}_i) \right)^2} + \expec{\left( m(\bm{X}_i) - f(\bm{X}_i) \right)^2} + 2 \expec{\left( y_i - m(\bm{X}_i) \right) \left( f(\bm{X}_i) - m(\bm{X}_i) \right)}
%   \end{align*}
% 
%   \begin{itemize}
...
%   \begin{align*}
%     & \expec{\left( y_i - m(\bm{X}_i) \right) \left( f(\bm{X}_i) - m(\bm{X}_i) \right)} \\
%     &\quad= \expec{\expec{\left( y_i - m(\bm{X}_i) \right) \left( f(\bm{X}_i) - m(\bm{X}_i) \right)}{\bm{X}_i}} \\
%     &\quad= \expec{\left( \expec{y_i}{\bm{X}_i} - m(\bm{X}_i) \right) \left( f(\bm{X}_i) - m(\bm{X}_i) \right)} \\
%     &\quad= \expec{\left( m(\bm{X}_i) - m(\bm{X}_i) \right) \left( f(\bm{X}_i) - m(\bm{X}_i) \right)} \\
%     &\quad= 0
%   \end{align*}
% \end{frame}
% 
...
%   The model $f$ that minimizes the mean-square prediction error is the conditional expectation function.
%   \begin{align*}
%     &\argmin_{f} \expec{\left( y_i - f(\bm{X}_i) \right)^2} = \expec{\left( y_i - m(\bm{X}_i) + m(\bm{X}_i) - f(\bm{X}_i) \right)^2} \\
%     &\quad= \argmin_{f} \expec{\left( y_i - m(\bm{X}_i) \right)^2} + \expec{\left( m(\bm{X}_i) - f(\bm{X}_i) \right)^2} + 0
%   \end{align*}
% 
%   \begin{itemize}
%     \item Minimizing this with respect to $f$ only involves the second term so we set $f(\bm{X}_i) = m(\bm{X}_i)$
%   \end{itemize}
% \end{frame}
% 
