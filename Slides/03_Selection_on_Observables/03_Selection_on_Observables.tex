...
\begin{frame}{What drives treatment after conditioning on $\bm{X}_i$}
  \bigskip
  \begin{quote}
    This says that for units with the same value of $\bm{X}_i = \bm{x}$, treatment is independent of the potential outcomes (i.e. ``randomly assigned'')
  \end{quote}
...
\begin{frame}{Matching estimator}
  ...
  \bigskip
  Each control unit has (approximately) the same $\bm{X}$ as it's respective treated unit, so under conditional independence, their $Y(0)$ is a good estimate (on average) for the treated unit's $Y(0)$.
\end{frame}
    
\begin{frame}{The `matched' control gropu}
  We selected the matched control units $\{ j(i): D_i = 1 \}$ so that they have the same distribution of $\bm{X}$ as the treated units
  
  \bigskip
  In this way, we have `created' a valid control group, i.e. removed selection on $\bm{X}$
  \begin{itemize}
    \item Once we match, we just go back to difference-in-means estimator
    
    \item Supposes there is no \emph{remaining} selection after matching on $\bm{X}$
  \end{itemize}
\end{frame}
...
\begin{frame}{Inference after Matching}
  ...
  \begin{itemize}
    \item Incoporating uncertainty from step 1 is a very difficult problem (see Imbens and Abadie, Econometrica 2006)
  \end{itemize}
\end{frame}
...
\section{Modelling $\prob{D_i = 1}{X_i}$}
...
\subsection{Approach 1: Propensity-score matching}
...
\subsection{Approach 2: Inverse probability of treatment weighting}
